 ...
 * PaymentManageView
 *
 * SRP: encapsulates the existing "manage payments" experience (search,
 * filters, add form, list). Extracted from `PaymentManager` so the parent
 * component only orchestrates tab switching.
 */
...
export default function PaymentManageView() {
  const {
    payments,
    createPayment,
    updatePayment,
  ...
  void unknownStudentLabel; // used by PaymentList through translations
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter((payment: Payment) => {
      const studentName = (
        studentsById.get(payment.student_id)?.name ?? payment.student_name ?? ''
      ).toLowerCase();
  ...
  const addPayment = async (data: PaymentFormData): Promise<boolean | undefined> => {
    try {
      ...
      if (result) {
        showSuccess(t('payments.actions.addSuccess', 'Payment added successfully'), {
          description: t('payments.actions.addSuccessDescription', 'The payment has been recorded.'),
        });
        return true;
      }
      showError(t('payments.actions.addError', 'Failed to add payment'), {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative">
              ...
              <label htmlFor="payments-manage-search" className="sr-only">
                {t('payments.search.placeholder')}
              </label>
              <Input
                id="payments-manage-search"
            ...
            <div>
              <label htmlFor="payments-manage-status" className="sr-only">
                {t('payments.filters.allStatuses')}
              </label>
              <Select
                id="payments-manage-status"
                value={statusFilter}
            ...
            <div>
              <label htmlFor="payments-manage-type" className="sr-only">
                {t('payments.filters.allTypes')}
              </label>
              <Select
                id="payments-manage-type"
                value={typeFilter}
